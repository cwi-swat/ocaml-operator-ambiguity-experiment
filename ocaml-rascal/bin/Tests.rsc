...
*/
public void run() {	 
	files = readFileLines(|file:///Users/ali/sle2013/toplevels.txt|);
	for (f <- files) {
		print(f + "...");
		try {
			tree = jparse(#start[TopLevel], readFile(|file:///Users/ali/sle2013| + f));
			...
			s = printAST(tree);
   			writeFile(|file:///Users/ali/sle2013/| + (f + ".rascal"), s);
			println("OK");
		}
		catch ParseError(el) : {
			println("Parse Error");
